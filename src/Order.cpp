...
Order::Order(string orderId, Stocks* stock, OrderType orderType, OrderPriceType priceType, 
...
OrderPriceType Order::getPriceType() const {
    return priceType;
}
...
time_t Order::getLastUpdated() const {
    return lastUpdated;
}
...
void Order::executeOrder(double executionPrice, int executedQuantity) {
    if (!canExecute() || executedQuantity <= 0) return;
    
    int actualExecutedQuantity = min(executedQuantity, getRemainingQuantity());
    
    // Update average fill price
    double totalFilledValue = (avgFillPrice * filledQuantity) + (executionPrice * actualExecutedQuantity);
...
void Order::partialFill(double fillPrice, int fillQuantity) {
    executeOrder(fillPrice, fillQuantity);
}
...
double Order::getFilledValue() const {
    return filledQuantity * avgFillPrice;
}

bool Order::isCompletelyFilled() const {
...
string Order::getOrderTypeString() const {
    switch (orderType) {
        ...
        case BUY_TO_COVER: return "BUY_TO_COVER";
        case SELL_SHORT: return "SELL_SHORT";
...
Position* Order::createPositionFromOrder() const {
    if (status != FILLED || stock == nullptr) return nullptr;
    
    // Determine position type
    PositionType posType = LONG;
    if (orderType == SELL_SHORT) {
        posType = SHORT;
    }
    
    // Determine quantity (negative for short positions)
    int posQuantity = filledQuantity;
    if (orderType == SELL || orderType == SELL_SHORT) {
        posQuantity = -filledQuantity;
    }
    
    // Create position using the stock data and new constructor
    return new Position(stock->getTicker(), stock->getCompanyName(), stock->getCurrentPrice(), 
                       posType, posQuantity, avgFillPrice);
}
...
void Order::printOrderDetails() const {
    ...
    if (stock != nullptr) {
        cout << "Stock: " << stock->getTicker() << " (" << stock->getCompanyName() << ")" << endl;
        cout << "Current Stock Price: $" << fixed << setprecision(2) << stock->getCurrentPrice() << endl;
    }
