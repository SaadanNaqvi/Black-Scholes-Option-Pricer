...
// Getters
string Portfolio::getPortfolioId() const {
    return portfolioId;
}

string Portfolio::getOwnerId() const {
    return ownerId;
}
...
vector<Order*> Portfolio::getOrders() const {
    return orders;
}
...
time_t Portfolio::getCreationDate() const {
    return creationDate;
}

time_t Portfolio::getLastUpdated() const {
    return lastUpdated;
}
...
void Portfolio::displayInfo() const {
    cout << "[PORTFOLIO] " << getClassType() << ": " << portfolioId 
         << " (Owner: " << ownerId << ", Value: $" 
         << fixed << setprecision(2) << calculateValue() << ")" << endl;
}

string Portfolio::getClassType() const {
    return "Portfolio";
}

void Portfolio::updateCurrentPrice(double price) {
    // For portfolio, update the inherited position (if any) and recalculate totals
...
vector<Order*> Portfolio::getPendingOrders() const {
    ...
    for (Order* order : orders) {
        if (order->getStatus() == PENDING || order->getStatus() == PARTIALLY_FILLED) {
...
void Portfolio::processOrders() {
    for (Order* order : orders) {
        if (order->canExecute()) {
            ...
            if ((order->getOrderType() == BUY || order->getOrderType() == BUY_TO_COVER)) {
            ...
            if (order->isCompletelyFilled()) {
                ...
                if (newPosition != nullptr) {
                    ...
                    if (hasPosition(ticker)) {
                        ...
                        existingPos->addToPosition(abs(newPosition->getQuantity()), newPosition->getEntryPrice());
                        delete newPosition;
                    } else {
...
double Portfolio::calculateDayChange() const {
    // Simplified calculation - would need historical data for accurate implementation
    double dayChange = 0.0;
    for (const auto& pair : positions) {
        double dailyReturn = pair.second->calculateDailyReturn();  // Use inherited method
...
vector<Position*> Portfolio::getTopPositions(int count) const {
    ...
    return vector<Position*>(allPositions.begin(), allPositions.begin() + actualCount);
}
...
void Portfolio::printOrders() const {
    ...
    int count = 0;
    for (auto it = orders.rbegin(); it != orders.rend() && count < 10; ++it, ++count) {
        Order* order = *it;
        if (order->getStock() != nullptr) {
            cout << order->getOrderTypeString() << " " 
                 << order->getQuantity() << " " 
...
Portfolio::~Portfolio() {
    ...
    // Note: We don't delete stocks from watchlist as they might be owned by other objects
    watchlist.clear();
}
